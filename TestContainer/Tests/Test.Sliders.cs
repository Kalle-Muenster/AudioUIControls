...
namespace MidiGUI.Test.Container
{
    public class Sliders
    {
        ...
        public void Test_GuiSlider()
        {
            ...
            test.StepInfo( "Set {0} range: 0 to 100", test.CurrentCase );
            ...
            test.StepInfo("Sliding down to bottom position");
            Click( Button.L|Button.DOWN );
            for( int i = 0; i < 50; ++i ) {
                point.Y += 10;
                Sleep(25);
            ...
            Sleep( 100 );
            Aut.SetControlValue( 50.0f );
            test.StepInfo("Reset Slider to value 50");
            test.StepInfo("Inverting Slider direction");
            ...
            Click( Button.L, test.InvertButton.Center );
            Sleep( 200 );
            test.StepInfo( "Reset Slider to (not inverted) regular direction" );

            testling.AddEventMarker(75, "Upper Mark", SliderMarkerPassed);
            testling.AddEventMarker(25, "Lower Mark", SliderMarkerPassed);

            counter = 0;
            test.StepInfo("Add two event markers: at value 25 and at value 75");
            test.StepInfo("Sliding to topmost position");
            point = area.Center.ToPoint();
            Mouse( Move.Absolute, point );
            Click( Button.L|Button.DOWN );
